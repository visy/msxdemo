...
; This file was generated Thu Mar  1 20:18:54 2018
;--------------------------------------------------------
	.module demo
	.optsdcc -mz80
...
;demo.c:932: uint8_t tripaltick = 0;
	ld	iy,#_tripaltick
	ld	0 (iy),#0x00
;demo.c:934: int triframes = 0;
	ld	iy,#_triframes
	ld	0 (iy),#0x00
	ld	iy,#_triframes
	ld	1 (iy),#0x00
;demo.c:1039: int sceneindex = 0;
	ld	iy,#_sceneindex
	ld	0 (iy),#0x00
	ld	iy,#_sceneindex
	ld	1 (iy),#0x00
;demo.c:1040: int timeindex = 0;
	ld	iy,#_timeindex
	ld	0 (iy),#0x00
	ld	iy,#_timeindex
	ld	1 (iy),#0x00
;demo.c:1042: void (*scenepointers[6])() = {
	...
	inc	hl
	ld	(hl),#>_animplay
;demo.c:1051: int scenetimings[12] = {
	ld	hl,#_scenetimings
	call	__initrleblock
	.db	24
...
;demo.c:843: if (bt >= 80) bt+=7;
	...
	ld	(hl),a
00108$:
;demo.c:844: if (bt < 80 && bt >= 60) bt+=6;
	ld	a,(#_bt+0)
	sub	a,#0x50
	ld	a,(#_bt+1)
	sbc	a,#0x00
	...
	ld	(hl),a
00110$:
;demo.c:845: if (bt < 60 && bt >= 40) bt+=5;
	ld	a,(#_bt+0)
	sub	a,#0x3C
	ld	a,(#_bt+1)
	sbc	a,#0x00
	...
	ld	(hl),a
00113$:
;demo.c:846: if (bt < 40 && bt >= 10) bt+=4;
	ld	a,(#_bt+0)
	sub	a,#0x28
	ld	a,(#_bt+1)
	sbc	a,#0x00
...
_tritiles:
	...
	add	hl,sp
	ld	sp,hl
;demo.c:943: if (tri_inited == 0) {
	xor	a,a
	ld	hl,#_tri_inited + 0
	or	a,(hl)
	jp	NZ,00102$
;demo.c:944: tri_inited = 1;
	ld	iy,#_tri_inited
	ld	0 (iy),#0x01
;demo.c:946: cmd.size_x = 256;
	ld	hl, #_cmd + 8
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x01
;demo.c:947: cmd.size_y = 1;
	ld	hl, #_cmd + 10
	ld	(hl),#0x01
	inc	hl
	ld	(hl),#0x00
...
;demo.c:950: cmd.command = 0xd0; // vram to vram, y only
	...
	ld	a,#0xD0
	ld	(bc),a
;demo.c:951: cmd.source_x = 0;
	ld	hl,#_cmd
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x00
;demo.c:952: cmd.source_y = 255;
	ld	hl, #_cmd + 2
	ld	(hl),#0xFF
	inc	hl
	ld	(hl),#0x00
;demo.c:953: cmd.dest_y = 0;
	...
	inc	hl
	ld	(hl),#0x00
;demo.c:955: for (x = 0; x < 212; x++) {
	ld	-2 (ix),#0xD4
	ld	-1 (ix),#0x00
00121$:
;demo.c:956: waitVB();
		halt 
;demo.c:957: cmd.dest_x = 0;
	ld	de,#_cmd + 4
	ex	de,hl
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x00
;demo.c:958: cmd.dest_y = x;
	ld	de,#_cmd + 6
	ld	c,-2 (ix)
	ld	b,-1 (ix)
	dec	bc
...
;demo.c:955: for (x = 0; x < 212; x++) {
	...
	or	a,-1 (ix)
	jr	NZ,00121$
;demo.c:962: vdp_register(9,2); // 50hz,192
	ld	hl,#0x0209
	push	hl
	call	_vdp_register
	pop	af
;demo.c:964: vdp_load_palette(boxes_palette);
	...
	call	_vdp_load_palette
	pop	af
;demo.c:966: msx2_palette(15,0,0,0);
	ld	hl,#0x0000
	push	hl
	ld	l, #0x0F
	push	hl
	...
	pop	af
	pop	af
;demo.c:968: drawtilescreen(tri_center);
	ld	hl,#_tri_center
	push	hl
	call	_drawtilescreen
	pop	af
00102$:
;demo.c:973: triframes++;
	ld	iy,#_triframes
	inc	0 (iy)
	jr	NZ,00142$
	ld	iy,#_triframes
...
00143$:
	jr	00104$
00144$:
;demo.c:975: drawtilescreen(tri_up);
	ld	hl,#_tri_up
	push	hl
	call	_drawtilescreen
	pop	af
00104$:
;demo.c:978: if (triframes == 400) {
	ld	a,(#_triframes+0)
	sub	a,#0x90
	jr	NZ,00145$
	ld	a,(#_triframes+1)
...
00145$:
	jr	00106$
00146$:
;demo.c:979: drawtilescreen(tri_dia);
	ld	hl,#_tri_dia
	push	hl
	call	_drawtilescreen
	pop	af
00106$:
;demo.c:981: if (triframes == 600) {
	ld	a,(#_triframes+0)
	sub	a,#0x58
	jr	NZ,00147$
	ld	a,(#_triframes+1)
...
00147$:
	jr	00108$
00148$:
;demo.c:982: drawtilescreen(tri_side1);
	ld	hl,#_tri_side1
	push	hl
	call	_drawtilescreen
	pop	af
00108$:
;demo.c:984: if (triframes == 700) {
	ld	a,(#_triframes+0)
	sub	a,#0xBC
	jr	NZ,00149$
	ld	a,(#_triframes+1)
...
00149$:
	jr	00110$
00150$:
;demo.c:985: drawtilescreen(tri_side2);
	ld	hl,#_tri_side2
	push	hl
	call	_drawtilescreen
	pop	af
00110$:
;demo.c:987: if (triframes == 800) {
	ld	a,(#_triframes+0)
	sub	a,#0x20
	jr	NZ,00151$
	ld	a,(#_triframes+1)
...
00151$:
	jr	00112$
00152$:
;demo.c:988: drawtilescreen(tri_side1);
	ld	hl,#_tri_side1
	push	hl
	call	_drawtilescreen
	pop	af
00112$:
;demo.c:990: if (triframes == 900) {
	ld	a,(#_triframes+0)
	sub	a,#0x84
	jr	NZ,00153$
	ld	a,(#_triframes+1)
...
00153$:
	jr	00114$
00154$:
;demo.c:991: drawtilescreen(tri_side2);
	ld	hl,#_tri_side2
	push	hl
	call	_drawtilescreen
	pop	af
00114$:
;demo.c:993: if (triframes == 1000) {
	ld	a,(#_triframes+0)
	sub	a,#0xE8
	jr	NZ,00155$
	ld	a,(#_triframes+1)
...
00155$:
	jr	00116$
00156$:
;demo.c:994: triframes = 0;
	ld	hl,#_triframes + 0
	ld	(hl), #0x00
	ld	hl,#_triframes + 1
	ld	(hl), #0x00
;demo.c:995: drawtilescreen(tri_center);
	ld	hl,#_tri_center
	push	hl
	call	_drawtilescreen
	pop	af
00116$:
;demo.c:998: msx2_palette(3,tripal[0],tripal[1],tripal[2]);
	ld	hl,#_tripal + 2
	ld	c,(hl)
	ld	de,#_tripal + 1
	ld	a,(de)
	...
	pop	af
	pop	af
;demo.c:999: msx2_palette(13,tripal[3],tripal[4],tripal[5]);
	ld	hl,#_tripal + 5
	ld	c,(hl)
	ld	de,#_tripal + 4
	ld	a,(de)
	...
	pop	af
	pop	af
;demo.c:1000: msx2_palette(5,tripal[6],tripal[7],tripal[8]);
	ld	hl,#_tripal + 8
	ld	c,(hl)
	ld	de,#_tripal + 7
	ld	a,(de)
	...
	pop	af
	pop	af
;demo.c:1003: tripaltick++;
	ld	iy,#_tripaltick
	inc	0 (iy)
;demo.c:1004: if (tripaltick > 6) {
	ld	a,#0x06
	ld	iy,#_tripaltick
	sub	a,0 (iy)
	jp	NC,00126$
;demo.c:1006: r = tripal[0];
	ld	hl,#_tripal
	ld	c,(hl)
	ld	-3 (ix),c
;demo.c:1007: g = tripal[1];
	ld	de,#_tripal + 1
	ld	a,(de)
	ld	-4 (ix),a
;demo.c:1008: b = tripal[2];
	ld	hl,#_tripal + 2
	ld	e,(hl)
	ld	-5 (ix),e
;demo.c:1010: for (x = 0; x < 6; x++) {
	ld	-2 (ix),#0x00
	ld	-1 (ix),#0x00
00122$:
	ld	a,-2 (ix)
	...
	sbc	a,#0x00
	jp	P,00125$
;demo.c:1011: tripal[x] = tripal[x+3];
	ld	a,#<_tripal
	add	a,-2 (ix)
	ld	c,a
	ld	a,#>_tripal
...
00157$:
	jr	00122$
00125$:
;demo.c:1014: tripal[6] = r;
	ld	bc,#_tripal + 6
	ld	a,-3 (ix)
	ld	(bc),a
;demo.c:1015: tripal[7] = g;
	ld	bc,#_tripal + 7
	ld	a,-4 (ix)
	ld	(bc),a
;demo.c:1016: tripal[8] = b;
	ld	bc,#_tripal + 8
	ld	a,-5 (ix)
	ld	(bc),a
;demo.c:1017: tripaltick = 0;
	ld	hl,#_tripaltick + 0
	ld	(hl), #0x00
00126$:
	ld	sp,ix
...
;demo.c:1027: waitVB();
		halt 
;demo.c:1028: uninstall_isr();
	call	_uninstall_isr
;demo.c:1029: PLY_Stop();
	call	_PLY_Stop
;demo.c:1030: PLY_SendRegisters();
	call	_PLY_SendRegisters
;demo.c:1032: screen(0);
	ld	a,#0x00
	push	af
	inc	sp
	call	_screen
	inc	sp
;demo.c:1034: puts("demo quit\r\n\r\n");
	ld	hl,#__str_13
	push	hl
	call	_puts
	pop	af
...
_main:
	...
	add	hl,sp
	ld	sp,hl
;demo.c:1061: unsigned char quit=0;
	ld	-1 (ix),#0x00
;demo.c:1067: spindown();
	call	_spindown
;demo.c:1069: puts("demo init\r\n\r\n");
	ld	hl,#__str_14
	push	hl
	call	_puts
	pop	af
;demo.c:1071: for (y = 0; y < 2; y++) {
	ld	-5 (ix),#0x00
	ld	-4 (ix),#0x00
00118$:
	ld	a,-5 (ix)
...
00114$:
	...
	sbc	a,#0x00
	jp	P,00120$
;demo.c:1073: tri_lookup_x[(y*16)+x] = x * 16;
	ld	a,-5 (ix)
	rlca
	rlca
	rlca
	...
	and	a,#0xF0
	ld	(bc),a
;demo.c:1074: tri_lookup_y[(y*16)+x] = (768+129)+(y * 16);
	sla	e
	ld	a,#<_tri_lookup_y
	add	a,e
	ld	l,a
...
00140$:
	jp	00118$
00121$:
;demo.c:1078: puts("music init...");
	ld	hl,#__str_15
	push	hl
	call	_puts
	pop	af
;demo.c:1080: PLY_SongPtr = (char *)0x0103;
	ld	hl,#_PLY_SongPtr + 0
	ld	(hl), #0x03
	ld	hl,#_PLY_SongPtr + 1
	ld	(hl), #0x01
;demo.c:1081: PLY_Init();
	call	_PLY_Init
;demo.c:1082: puts("done.\r\n");
	ld	hl,#__str_16
	push	hl
	call	_puts
	pop	af
;demo.c:1084: puts("detecting vdp type...\r\n\r\n");
	ld	hl,#__str_17
	push	hl
	call	_puts
	pop	af
;demo.c:1085: if(isvdp2())
	call	_isvdp2
	xor	a,a
	or	a,l
	jr	Z,00102$
;demo.c:1087: puts("vdp2 found! setting 50hz...\r\n\r\n");
	ld	hl,#__str_18
	push	hl
	call	_puts
	pop	af
...
;demo.c:1091: do_quit();
	call	_do_quit
00103$:
;demo.c:1094: pal_load("TWISTER PL5",32,1);
	ld	hl,#0x0001
	push	hl
	ld	a,#0x20
	push	af
	...
	pop	af
	inc	sp
;demo.c:1095: memcpy(twister_palette, cur_palette, 32);
	ld	hl,#0x0020
	push	hl
	ld	hl,#_cur_palette
	push	hl
	...
	pop	af
	pop	af
;demo.c:1096: pal_load("LF      PL5",32,1);
	ld	hl,#0x0001
	push	hl
	ld	a,#0x20
	push	af
	...
	pop	af
	inc	sp
;demo.c:1097: memcpy(tf_palette, cur_palette, 32);
	ld	hl,#0x0020
	push	hl
	ld	hl,#_cur_palette
	push	hl
	...
	pop	af
	pop	af
;demo.c:1098: pal_load("BOXES   PL5",32,1);
	ld	hl,#0x0001
	push	hl
	ld	a,#0x20
	push	af
	...
	pop	af
	inc	sp
;demo.c:1099: memcpy(boxes_palette, cur_palette, 32);
	ld	hl,#0x0020
	push	hl
	ld	hl,#_cur_palette
	push	hl
	...
	pop	af
	pop	af
;demo.c:1100: pal_load("BULBS   PL5",32,1);
	ld	hl,#0x0001
	push	hl
	ld	a,#0x20
	push	af
	...
	pop	af
	inc	sp
;demo.c:1102: scratch_clear();
	call	_scratch_clear
;demo.c:1103: vdp_load_palette(scratch);
	ld	hl,#_scratch
	push	hl
	call	_vdp_load_palette
	pop	af
;demo.c:1105: vdp_set_screen5();
	call	_vdp_set_screen5
;demo.c:1107: pck_load("BULBS   PCK",2431,0x0000,VRAM_0,0);
	ld	hl,#0x0000
	push	hl
	ld	l, #0x01
	push	hl
	...
	add	iy,sp
	ld	sp,iy
;demo.c:1109: cmd.size_x = 256;
	ld	hl, #_cmd + 8
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x01
;demo.c:1110: cmd.size_y = 212;
	ld	hl, #_cmd + 10
	ld	(hl),#0xD4
	inc	hl
	ld	(hl),#0x00
;demo.c:1111: cmd.data = 0;
;demo.c:1112: cmd.argument = 0x00;
	ld	a,#0x00
	ld	(#_cmd + 12),a
	ld	(#_cmd + 13),a
;demo.c:1113: cmd.command = 0xd0; 
	ld	bc,#_cmd + 14
	ld	a,#0xD0
	ld	(bc),a
;demo.c:1114: cmd.source_x = 0;
	ld	hl,#_cmd
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x00
;demo.c:1115: cmd.source_y = 0;
	ld	hl, #_cmd + 2
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x00
;demo.c:1116: cmd.dest_x = 0;
	ld	hl, #_cmd + 4
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x00
;demo.c:1117: cmd.dest_y = 256;
	ld	hl, #_cmd + 6
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x01
;demo.c:1118: vdp_copier(&cmd);
	ld	hl,#_cmd
	push	hl
	call	_vdp_copier
	pop	af
;demo.c:1120: pck_load("TWISTER PCK",4032,0x0000,VRAM_0,0);
	ld	hl,#0x0000
	push	hl
	ld	l, #0x01
	push	hl
...
;demo.c:1131: vdp_copier(&cmd);
	...
	call	_vdp_copier
	pop	af
;demo.c:1134: pck_load("BOXES   PCK",2098,0x0000,VRAM_0,0);
	ld	hl,#0x0000
	push	hl
	ld	l, #0x01
	push	hl
	...
	add	iy,sp
	ld	sp,iy
;demo.c:1135: cmd.size_x = 256;
	ld	hl, #_cmd + 8
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x01
;demo.c:1136: cmd.size_y = 212;
	ld	hl, #_cmd + 10
	ld	(hl),#0xD4
	inc	hl
	ld	(hl),#0x00
;demo.c:1137: cmd.data = 0;
;demo.c:1138: cmd.argument = 0x00;
	ld	a,#0x00
	ld	(#_cmd + 12),a
	ld	(#_cmd + 13),a
;demo.c:1139: cmd.command = 0xd0; 
	ld	bc,#_cmd + 14
	ld	a,#0xD0
	ld	(bc),a
;demo.c:1140: cmd.source_x = 0;
	ld	hl,#_cmd
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x00
;demo.c:1141: cmd.source_y = 0;
	ld	hl, #_cmd + 2
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x00
;demo.c:1142: cmd.dest_x = 0;
	ld	hl, #_cmd + 4
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x00
;demo.c:1143: cmd.dest_y = 768;
	ld	hl, #_cmd + 6
	ld	(hl),#0x00
	inc	hl
	ld	(hl),#0x03
;demo.c:1144: vdp_copier(&cmd);
	ld	hl,#_cmd
	push	hl
	call	_vdp_copier
	pop	af
;demo.c:1146: scratch_clear();
	call	_scratch_clear
;demo.c:1147: vdp_load_palette(scratch);
	ld	hl,#_scratch
	push	hl
	call	_vdp_load_palette
	pop	af
;demo.c:1149: pck_load("DSSLOGO PCK",2154,0x0000,VRAM_0,0);
	ld	hl,#0x0000
	push	hl
	ld	l, #0x01
	push	hl
	...
	add	iy,sp
	ld	sp,iy
;demo.c:1150: pal_load("DSSLOGO PL5",32,0);
	ld	hl,#0x0000
	push	hl
	ld	a,#0x20
	push	af
	...
	pop	af
	inc	sp
;demo.c:1152: scratch_clear();
	call	_scratch_clear
;demo.c:1154: vdp_register(9,130); // 50hz,212
	ld	hl,#0x8209
	push	hl
	call	_vdp_register
	pop	af
;demo.c:1156: install_isr(my_isr);
	ld	hl,#_my_isr
	push	hl
	call	_install_isr
	pop	af
;demo.c:1158: while (!quit) {
00111$:
	xor	a,a
	or	a,-1 (ix)
	jp	NZ,00113$
;demo.c:1160: if (vbicount >= scenetimings[timeindex+1]) {
	ld	hl,#_timeindex + 0
	ld	c,(hl)
	inc	c
	sla	c
...
;demo.c:1162: timeindex+=2;
	...
	ld	(hl),a
00105$:
;demo.c:1165: if (vbicount >= scenetimings[timeindex] && vbicount < scenetimings[timeindex+1]) {
	ld	hl,#_timeindex + 0
	ld	c,(hl)
	ld	hl,#_timeindex + 1
	ld	b,(hl)
...
;demo.c:1166: waitVB();
		halt 
;demo.c:1167: scenepointers[sceneindex]();
	ld	hl,#_sceneindex + 0
	ld	c,(hl)
	ld	iy,#_sceneindex
	ld	b,1 (iy)
