...
; This file was generated Wed Feb  7 21:54:22 2018
;--------------------------------------------------------
	.module demo
	.optsdcc -mz80
...
00107$:
	...
	ret
_pal_load_end::
;demo.c:97: void pause() {
;	---------------------------------
; Function pause
; ---------------------------------
_pause_start::
_pause:
;demo.c:98: uint8_t i,j,k = 0;
;demo.c:99: for (i = 0; i < 255; i++) {	
	ld	bc,#0x0000
00104$:
	ld	a,b
	sub	a,#0xFF
	ret	NC
;demo.c:100: for (j = 0; j < 255; j++) {
	ld	e,c
	ld	d,#0xFF
00103$:
;demo.c:101: k++;
...
;demo.c:99: for (i = 0; i < 255; i++) {	
	...
	jr	00104$
_pause_end::
;demo.c:106: void cls(uint8_t vramh, uint16_t vraml) {
;	---------------------------------
; Function cls
; ---------------------------------
_cls_start::
_cls:
	...
	ld	ix,#0
	add	ix,sp
;demo.c:107: vdp_set_write_address(vramh, vraml);
	ld	l,5 (ix)
	ld	h,6 (ix)
	push	hl
	ld	a,4 (ix)
	...
	pop	af
	inc	sp
;demo.c:108: memset((uint8_t *) &scratch, 0, 128);
	ld	hl,#0x0080
	push	hl
	ld	a,#0x00
	push	af
	...
	pop	af
	inc	sp
;demo.c:109: vdp_load_screen(scratch, 128);
	ld	a,#0x80
	push	af
	inc	sp
	ld	hl,#_scratch
...
;demo.c:117: tick = 0;
	...
	ld	hl,#_tick + 1
	ld	(hl), #0x00
;demo.c:119: for(i = 0; i < 32; i+=2) {
	ld	c,#0x00
00109$:
	ld	a,c
	sub	a,#0x20
	jp	NC,00112$
;demo.c:120: uint8_t r = scratch[i] >> 4;
	ld	a,#<_scratch
	add	a,c
	ld	e,a
	ld	a,#>_scratch
	...
	srl	e
	srl	e
;demo.c:121: uint8_t b = scratch[i] & 0xf;
	ld	a,b
	and	a,#0x0F
	ld	-1 (ix),a
;demo.c:122: uint8_t g = scratch[i+1];
	ld	d,c
	inc	d
	ld	a,#<_scratch
	add	a,d
	...
	ld	d,(hl)
	ld	-2 (ix),d
;demo.c:124: if (r < (cur_palette[i] >> 4)) r++;
	ld	a,#<_cur_palette
	add	a,c
	ld	l,a
	ld	a,#>_cur_palette
	...
	inc	e
00104$:
;demo.c:125: if (b < (cur_palette[i] & 0xf)) b++;
	ld	a,#<_cur_palette
	add	a,c
	ld	l,a
	ld	a,#>_cur_palette
...
;demo.c:127: scratch[i] = (r << 4) | b;
	...
	ld	h,d
	ld	(hl),a
;demo.c:128: if (g < cur_palette[i+1]) scratch[i+1]++;
	ld	b,c
	inc	b
	ld	a,#<_cur_palette
	add	a,b
...
;demo.c:119: for(i = 0; i < 32; i+=2) {
	...
	jp	00109$
00112$:
;demo.c:132: vdp_load_palette(scratch);
	ld	hl,#_scratch
	push	hl
...
;demo.c:138: spindown();
	call	_spindown
;demo.c:140: puts("demo init\r\n\r\n");
	ld	hl,#__str_0
	push	hl
	call	_puts
	pop	af
;demo.c:142: puts("music init...");
	ld	hl,#__str_1
	push	hl
	call	_puts
	pop	af
;demo.c:144: PLY_SongPtr = (char *)0x0103;
...
;demo.c:145: PLY_Init();
	call	_PLY_Init
;demo.c:146: puts("done.\n\n");
	ld	hl,#__str_2
	push	hl
	call	_puts
	pop	af
;demo.c:148: if(isvdp2())
	call	_isvdp2
	xor	a,a
	or	a,l
	jr	Z,00102$
;demo.c:150: msx2_sethz(50);
	ld	a,#0x32
	push	af
	inc	sp
	call	_msx2_sethz
	inc	sp
00102$:
;demo.c:155: vdp_set_screen6();
	call	_vdp_set_screen6
;demo.c:157: pal_load("MONOLOG PL6", 8);
	ld	a,#0x08
	push	af
	inc	sp
	ld	hl,#__str_3
	...
	pop	af
	inc	sp
;demo.c:158: vdp_load_palette(cur_palette);
	ld	hl,#_cur_palette
	push	hl
	call	_vdp_load_palette
	pop	af
;demo.c:160: ge5_load("MONOLOG SC6", 0, 0x0000);
	ld	hl,#0x0000
	push	hl
	ld	a,#0x00
	push	af
