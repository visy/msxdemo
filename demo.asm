...
; This file was generated Tue Feb 13 20:39:37 2018
;--------------------------------------------------------
	.module demo
	.optsdcc -mz80
...
;demo.c:306: volatile int linenum = 10;
	...
	ld	iy,#_linenum
	ld	1 (iy),#0x00
;demo.c:341: static int xo = 0;
	ld	iy,#_xo
	ld	0 (iy),#0x00
	ld	iy,#_xo
	ld	1 (iy),#0x00
;demo.c:342: static int yo = 0;
	ld	iy,#_yo
	ld	0 (iy),#0x00
	ld	iy,#_yo
	ld	1 (iy),#0x00
;demo.c:343: static int po = 0;
	ld	iy,#_po
	ld	0 (iy),#0x00
	ld	iy,#_po
	ld	1 (iy),#0x00
;demo.c:344: static int fffaaa = 0;
	ld	iy,#_fffaaa
	ld	0 (iy),#0x00
	ld	iy,#_fffaaa
	ld	1 (iy),#0x00
...
;demo.c:225: ymmmf++;
	...
	inc	1 (iy)
00116$:
;demo.c:226: for (yofff = 0; yofff<212-step;yofff+=step) {
	ld	hl,#_yofff + 0
	ld	(hl), #0x00
00105$:
	ld	a,(#_yofff+0)
	...
	sbc	a,#0x00
	jp	P,00109$
;demo.c:227: xo = (sintabx[(yofff+ymmmf) & 255]);
	ld	hl,#_ymmmf
	ld	a,-19 (ix)
	add	a,(hl)
	ld	e,a
...
;demo.c:230: cmd.source_x = 0;
	...
	inc	hl
	ld	(hl),#0x00
;demo.c:231: cmd.source_y = 0+yofff;
	ld	hl,#0x0006
	add	hl,sp
	ex	de,hl
	ld	c,e
...
;demo.c:352: cmd.source_x = xo;
	...
	inc	hl
	ld	(hl),b
;demo.c:353: cmd.source_y = 256+po+yo+(y>>1);
	ld	hl,#0x0004
	add	hl,sp
	ld	c,l
	ld	b,h
...
;demo.c:351: for (y = 0; y < 106; y+=1) {
	...
	jp	00107$
00110$:
;demo.c:364: for (y = 0; y < 106; y+=1) {
	ld	-1 (ix),#0x00
00111$:
	ld	a,-1 (ix)
	sub	a,#0x6A
	jp	NC,00114$
;demo.c:365: cmd.source_x = xo;
	ld	hl,#0x0004
	add	hl,sp
	ex	de,hl
	ld	bc,(_xo)
	...
	inc	hl
	ld	(hl),b
;demo.c:366: cmd.source_y = 256+po+yo+(y>>1);
	ld	hl,#0x0004
	add	hl,sp
	ld	c,l
	ld	b,h
	...
	inc	hl
	ld	(hl),d
;demo.c:367: cmd.dest_x = 128;
	ld	hl,#0x0004
	add	hl,bc
	ld	(hl),#0x80
	inc	hl
	ld	(hl),#0x00
;demo.c:368: cmd.dest_y = (y<<1);
	ld	hl,#0x0006
	add	hl,bc
	ld	-20 (ix),l
	ld	-19 (ix),h
	...
	inc	hl
	ld	(hl),d
;demo.c:369: cmd.size_x = 128;
	ld	hl,#0x0008
	add	hl,bc
	ld	(hl),#0x80
	inc	hl
	ld	(hl),#0x00
;demo.c:370: cmd.size_y = 1;
	ld	hl,#0x000A
	add	hl,bc
	ld	(hl),#0x01
	inc	hl
	ld	(hl),#0x00
;demo.c:371: cmd.data = 0;
	ld	hl,#0x000C
	add	hl,bc
	ex	de,hl
	ld	a,#0x00
	ld	(de),a
;demo.c:372: cmd.argument = 0x00;
	ld	hl,#0x000D
	add	hl,bc
	ex	de,hl
	ld	a,#0x00
	ld	(de),a
;demo.c:373: cmd.command = 0xd2; // logical vram to vram, xor
	ld	hl,#0x000E
	add	hl,bc
	ex	de,hl
	ld	a,#0xD2
	ld	(de),a
;demo.c:374: vdp_copier(&cmd);
	push	bc
	call	_vdp_copier
	pop	af
;demo.c:364: for (y = 0; y < 106; y+=1) {
...
;demo.c:381: yo+=53;
	...
	inc	hl
	ld	(hl),a
;demo.c:382: if (yo >= 212) { yo = 0; po+=256; }
	ld	a,(#_yo+0)
	sub	a,#0xD4
	ld	a,(#_yo+1)
	sbc	a,#0x00
...
;demo.c:384: po = 0;
	...
	ld	(hl), #0x00
00106$:
;demo.c:389: msx2_palette(15,3+sintab[fffaaa & 255]>>5,3,4);
	ld	hl,#_fffaaa + 0
	ld	c,(hl)
	ld	b,#0x00
	ld	hl,#_sintab
	...
	pop	af
	pop	af
;demo.c:390: msx2_palette(3,2+sintab[fffaaa & 255]>>5,0,0);
	ld	hl,#_fffaaa + 0
	ld	c,(hl)
	ld	b,#0x00
	ld	hl,#_sintab
...
00132$:
	...
	ret
_do_animplay_end::
;demo.c:400: void main() {
;	---------------------------------
; Function main
; ---------------------------------
_main_start::
_main:
;demo.c:401: unsigned char quit=0;
	ld	c,#0x00
;demo.c:402: int modes = 128; // interlace bit on
	ld	de,#0x0080
;demo.c:405: spindown();
	push	bc
	push	de
...
;demo.c:420: puts("music init...");
	...
	pop	de
	pop	bc
;demo.c:422: PLY_SongPtr = (char *)0x0103;
...
;demo.c:426: if(isvdp2())
	...
	or	a,a
	jr	Z,00102$
;demo.c:428: modes+=2; // pal
	ld	de,#0x0082
00102$:
;demo.c:432: vdp_set_screen5();
	push	bc
	push	de
	call	_vdp_set_screen5
	pop	de
	pop	bc
;demo.c:434: vdp_register(VDP_MODE3,modes); // interlace on, screen mode pal or ntsc
	ld	b,e
	push	bc
	push	bc
	inc	sp
...
;demo.c:436: puts("demo start\r\n");
	...
	pop	af
	pop	bc
;demo.c:438: scratch_clear();
	push	bc
	call	_scratch_clear
	pop	bc
;demo.c:439: vdp_load_palette(scratch);
	push	bc
	ld	hl,#_scratch
	push	hl
	call	_vdp_load_palette
	pop	af
	pop	bc
;demo.c:441: vdp_register(VDP_VOFFSET,0);
	push	bc
	ld	hl,#0x0017
	push	hl
	call	_vdp_register
...
;demo.c:443: pal_load("LF      PL5",32);
	...
	inc	sp
	pop	bc
;demo.c:444: vdp_load_palette(cur_palette);
	push	bc
	ld	hl,#_cur_palette
	push	hl
	call	_vdp_load_palette
	pop	af
	pop	bc
;demo.c:446: memset((uint8_t *) &packbuffer, 0, 13428);
	push	bc
	ld	hl,#0x3474
	push	hl
	ld	a,#0x00
	...
	inc	sp
	pop	bc
;demo.c:447: raw_load("LF1     PCK", 13428, packbuffer);
	push	bc
	ld	hl,#_packbuffer
	push	hl
	ld	hl,#0x3474
	...
	pop	af
	pop	bc
;demo.c:448: bitbuster(packbuffer,0x8000,VRAM_0); // to page 1
	push	bc
	ld	a,#0x01
	push	af
	inc	sp
	...
	inc	sp
	pop	bc
;demo.c:450: memset((uint8_t *) &packbuffer, 0, 14505);
	push	bc
	ld	hl,#0x38A9
	push	hl
	ld	a,#0x00
	...
	inc	sp
	pop	bc
;demo.c:451: raw_load("LF2     PCK", 14505, packbuffer);
	push	bc
	ld	hl,#_packbuffer
	push	hl
	ld	hl,#0x38A9
	...
	pop	af
	pop	bc
;demo.c:452: bitbuster(packbuffer,0x0000,VRAM_1); // to page 2
	push	bc
	ld	a,#0x00
	push	af
	inc	sp
	...
	inc	sp
	pop	bc
;demo.c:454: memset((uint8_t *) &packbuffer, 0, 14480);
	push	bc
	ld	hl,#0x3890
	push	hl
	ld	a,#0x00
	...
	inc	sp
	pop	bc
;demo.c:455: raw_load("LF3     PCK", 14480, packbuffer);
	push	bc
	ld	hl,#_packbuffer
	push	hl
	ld	hl,#0x3890
	...
	pop	af
	pop	bc
;demo.c:456: bitbuster(packbuffer,0x8000,VRAM_1); // to page 3
	push	bc
	ld	a,#0x00
	push	af
	inc	sp
...
;demo.c:469: install_isr(my_isr);
	...
	pop	af
	pop	bc
;demo.c:492: while (!quit) {
00105$:
	xor	a,a
	or	a,c
	jr	NZ,00107$
;demo.c:495: do_animplay();
	push	bc
	call	_do_animplay
	pop	bc
;demo.c:507: if(space())
...
;demo.c:511: waitVB();
		halt 
;demo.c:512: uninstall_isr();
	call	_uninstall_isr
;demo.c:513: PLY_Stop();
	call	_PLY_Stop
;demo.c:514: PLY_SendRegisters();
	call	_PLY_SendRegisters
;demo.c:516: screen(0);
	ld	a,#0x00
	push	af
