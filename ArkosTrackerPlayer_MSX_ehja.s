...
PLY_UseCPCMachine = 1		;Indicates what frequency table and output code to use. 1 to use it.
PLY_UseMSXMachine = 0


PLY_UseSoundEffects = 0	;Set to 1 if you want to use Sound Effects in your player. Both CPU and memory consuming.
PLY_UseFades = 0		;Set to 1 to allow fades in/out. A little CPU and memory consuming.
				;PLY_SetFadeValue becomes available.


PLY_SystemFriendly = 0	;Set to 1 if you want to save the Registers used by AMSDOS (AF', BC', IX, IY)
...
PLY_UseFirmwareInterruptions = 0 ;Set to 1 to use a Player under interruption. Only works on CPC, as it uses the CPC Firmware.
...
PLY_UseBasicSoundEffectInterface = 0	;Set to 1 if you want a little interface to be added if you are a BASIC programmer who wants
...
PLY_RetrigValue	= 0xfe		;Value used to trigger the Retrig of Register 13. #FE corresponds to CP xx. Do not change it !
...
;	exx
;PLY_SFX_Track3_Volume = $+2
;PLY_SFX_Track3_Note = $+1
;	ld de,0				;D=Inverted Volume E=Note
...
;	exx
;PLY_SFX_Track2_Volume = $+2
;PLY_SFX_Track2_Note = $+1
;	ld de,0				;D=Inverted Volume E=Note
...
;	exx
;PLY_SFX_Track1_Volume = $+2
;PLY_SFX_Track1_Note = $+1
;	ld de,0				;D=Inverted Volume E=Note
...
;	or ixl			;Mix Reg7 from Track3 with Track2+1.
    ...
    ret ; in our two-phase MSX player we don't send the PSG registers right away. You need to call it separately!
...
;	ld a,b				;Use the stored Register 7.
    ld a, (PLY_PSGReg7)  ; in our two-phase MSX player we must store all registers in the array
	out (0xa1),a
...
_PLY_PSGRegistersArray:
PLY_PSGRegistersArray:
PLY_PSGReg0: .db 0 ; frequency channel A, low
PLY_PSGReg1: .db 0 ; frequency channel A, high
PLY_PSGReg2: .db 0 ; frequency channel B, low
PLY_PSGReg3: .db 0 ; frequency channel B, high
PLY_PSGReg4: .db 0 ; frequency channel C, low
PLY_PSGReg5: .db 0 ; frequency channel C, high
PLY_PSGReg6: .db 0 ; noise period
_PLY_PSGReg8::
PLY_PSGReg8: .db 0		;+7    ; volume channel A
	    .db 0
_PLY_PSGReg9::
PLY_PSGReg9: .db 0		;+9    ; volume channel B
	    .db 0
_PLY_PSGReg10::
PLY_PSGReg10: .db 0		;+11   ; volume channel C
PLY_PSGReg11: .db 0   ; envelope frequency, low
PLY_PSGReg12: .db 0   ; envelope frequency, high 
PLY_PSGReg13: .db 0   ; envelope shape
PLY_PSGRegistersArray_End:

_PLY_PSGReg7::
PLY_PSGReg7: .db 0 ; mixer
...
;**************
	...
	ld 7(iy),a			;We have to make the volume to 0, because if a bass Hard was activated before, we have to stop it.
;	ld ixl,#9;%1001
    .db 0xDD
    ld  l, #9
...
;	ld ixl,#8;%1000	;By defaut, No Noise, Sound.
    .db 0xDD
    ld  l,#8
	...
	ld (PLY_PSGReg6),a
;	ld ixl,#0;%0000					;Open Noise Channel.
    .db 0xDD
    ld  l, #0
...
;	inc ixl						;Set Sound bit to stop the Sound.
    .db 0xDD   ; as-z80 doesn't support undocumented z80 instructions, so we need to do this "by hand" 
    inc l
	ret
...
;	ld ixl,#8;%1000					;Sound is always On here (only Independence mode can switch it off).
    .db 0xDD
    ld  l, #8
	...
	and #15;%1111			;We don't care about the bit 7-4, but we have to clear them, else the waveform might be reset.
	ld (PLY_PSGReg13),a
...
; ***** THIS IS FOR SDCC *****
; song address parameter is on stack at ix+4
;    ld      e,4(ix)
;    ld      d,5(ix)
; ****************************
...
;PLY_SFX_OffsetPitch = 0
;PLY_SFX_OffsetVolume = PLY_SFX_Track1_Volume - PLY_SFX_Track1_Pitch
;PLY_SFX_OffsetNote = PLY_SFX_Track1_Note - PLY_SFX_Track1_Pitch
;PLY_SFX_OffsetInstrument = PLY_SFX_Track1_Instrument - PLY_SFX_Track1_Pitch
;PLY_SFX_OffsetSpeed = PLY_SFX_Track1_InstrumentSpeed - PLY_SFX_Track1_Pitch
;PLY_SFX_OffsetSpeedCpt = PLY_SFX_Track1_InstrumentSpeedCpt - PLY_SFX_Track1_Pitch
